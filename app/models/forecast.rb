class Forecast
  attr_reader :id, :location, :currently_data, :hourly_data, :daily_data
  ...
  def currently
    {
      location: location,
      time: Time.at(currently_data[:time]).to_datetime.strftime('%I:%M%p %m/%d/%y'),
  ...
  def hourly
    hourly_data.map do |index|
      {
        time: Time.at(index[:time]).to_datetime.strftime('%I:%M%p'),
        icon: index[:icon],
  ...
  def daily
    daily_data[1..5].map do |index|
      {
        time: Time.at(index[:time]).to_datetime.strftime('%A'),
        icon: index[:icon],
